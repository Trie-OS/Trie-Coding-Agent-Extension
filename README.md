...
Brought to you by Trie IDE (formerly LocalForge), the open-source desktop coding IDE powered entirely by open models you download from Hugging Face — no cloud, no API keys, no subscription required.

## Why this is different

- **Local models, easily.** Point the extension at the Trie IDE daemon (`localforged`) and pick any GGUF model from your Trie IDE model store — external drive or internal folder. One command: *Trie IDE: Connect & Load Local Model*. Generation is grammar-constrained, so even 7–14B models emit well-formed tool calls reliably.
...
| `trie-ide.daemon.url` | `http://127.0.0.1:7841` | localforged base URL |
